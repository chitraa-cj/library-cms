...
- **CMS**: Strapi CMS running on AWS EC2 (13.53.121.15:1337 / [https://admincms.xoidlabs.com](https://admincms.xoidlabs.com))
...
- `cms_ocr_jobs` / `cms_ocr_job_chunks` - OCR Docs: one row per uploaded document, one row per page range (status, attempts, model, markdown, tokens). Created on first use by `ensureOcrSchema()`; see docs/OCR.md
...
- `server/ocr/` - OCR Docs (admin only): `config` (caps), `pdf` (page-range slicing), `limiter` (concurrency + per-minute + per-day gate), `gemini` (one request per range), `jobs` (worker pool, fallback models, resume), `routes` (`/api/admin/ocr`), `store` (rows + uploaded files). UI: `client/src/pages/admin-ocr.tsx`, renderer `client/src/lib/ocr-markdown.ts`. Full write-up: docs/OCR.md
...
- `STRAPI_URL` - Strapi CMS URL ([http://13.53.121.15:1337](http://13.53.121.15:1337))
...
- `GEMINI_API_KEY` - Gemini key for OCR Docs (admin-only transcription). Optional: `OCR_*` tuning vars (chunk size, concurrency, per-minute/per-day caps, fallback models) — see .env.example and docs/OCR.md. NOTE: a free-tier key is metered per model per day (~20 requests), enough for a few dozen pages only; enable billing for book-length documents.
...
`tests/ocr-pipeline.test.mjs` — run with `node --import tsx tests/ocr-pipeline.test.mjs`
(add `--live` for one real Gemini request). Covers chunk planning, PDF page-range
slicing, page-marker parsing, escape-first Markdown rendering and the rate limiter.
Full path against the local DB: `OCR_E2E_NO_SSL=1 node --import tsx script/ocr-e2e-local.mts <file.pdf>`.
